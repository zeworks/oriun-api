...
import { LoadDepartmentByNameUseCase, LoadDepartmentByNameUseCaseFunction } from "@/domain/usecases/departments/load-department-by-name";
...
export class DbLoadDepartmentByName implements LoadDepartmentByNameUseCase {
  ...
  loadByName: LoadDepartmentByNameUseCaseFunction = async (departmentName) => {
    try {
      const data = await this.loadDepartmentByNameRepository.loadByName(departmentName);

      if (!data)
        throw new Error("department name invalid")
