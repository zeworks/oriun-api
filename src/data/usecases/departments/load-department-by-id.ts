...
import { LoadDepartmentByIdUseCase, LoadDepartmentByIdUseCaseFunction } from "@/domain/usecases/departments/load-department-by-id";
...
export class DbLoadDepartmentById implements LoadDepartmentByIdUseCase {
  ...
  loadById: LoadDepartmentByIdUseCaseFunction = async (departmentId) => {
    try {
      const data = await this.loadDepartmentByIdRepository.loadById(departmentId);
