...
test("Should delete department with success", async () => {
  ...
  const result = await new DbDeleteDepartment(departmentsRepository, departmentsRepository).delete(department?.id!);
...
test("Should throw an error if trying to delete an invalid department id", async () => {
  const departmentsRepository = new InMemoryDepartmentsRepository();
  expect(new DbDeleteDepartment(departmentsRepository, departmentsRepository).delete("123")).rejects.toThrow(new Error("department id invalid"));
})
