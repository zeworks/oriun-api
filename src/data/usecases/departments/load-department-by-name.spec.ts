...
test("load department by name with success", async () => {
  ...
  const firstDepartment = await loadDepartmentByName().loadByName("Department Name 1");
  const secondDepartment = await loadDepartmentByName().loadByName("Department Name 2");
...
test("Should throw an error on load department name invalid", async () => {
  expect(loadDepartmentByName().loadByName("invalid name")).rejects.toThrow(new Error("department name invalid"));
})
