...
export class LoadDepartmentsController implements Controller {
	constructor(private readonly loadDepartments: LoadDepartmentsUseCase) {}

	execute = async (
		request?: LoadDepartmentsController.Params
	): Promise<HttpResponse<LoadDepartmentsUseCase.Result>> => {
		try {
