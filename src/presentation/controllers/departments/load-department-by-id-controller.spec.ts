...
test("Should load a department by id with success", async () => {
  ...
  const loadDepartmentByIdUseCase = new DbLoadDepartmentById(departmentsRepository);
  const loadDepartmentByIdController = new LoadDepartmentByIdController(makeLoadDepartmentByIdValidation(), loadDepartmentByIdUseCase);

  const result = await loadDepartmentByIdController.execute({ id: department.data!.id })
