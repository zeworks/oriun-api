...
test("Should update department name with success", async () => {
	...
	const updateDepartmentUseCase = new DbUpdateDepartment(
		departmentsRepository,
		departmentsRepository,
		departmentsRepository
	)
	const updateDepartment = new UpdateDepartmentController(
		makeUpdateDepartmentValidation(),
	...
	if (departmentCreated.data?.id) {
		...
		expect(departmentUpdated.data?.createdAt).toEqual(
			departmentCreated.data.createdAt
		)
	}
})
