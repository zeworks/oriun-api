...
export class LoadCompaniesController implements Controller {
  constructor(
    private readonly loadCompanies: LoadCompaniesUseCase
  ) { }

  execute: ControllerProtocol<LoadCompaniesController.Params, LoadCompaniesController.Result> = async (request) => {
    try {
