...
export class LoadCompanyByCodeController implements Controller {
  ...
  execute: ControllerProtocol<LoadCompanyByCodeController.Params, LoadCompanyByCodeController.Result> = async (request) => {
    ...
    try {
      const result = await this.loadCompanyByCodeUseCase.loadByCode(request!.code);
      return ok(result);
    } catch (error: any) {
