...
test("Should load the account details with success", async () => {
  ...
  const createAccountDb = new DbCreateAccount(uuidAdapter, encrypter, usersRepository, usersRepository, usersRepository);
  const createAccountController = new CreateAccountController(makeCreateAccountValidation(), createAccountDb);
