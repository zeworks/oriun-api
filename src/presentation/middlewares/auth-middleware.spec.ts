...
test("Should have valid accesstoken", async () => {
  ...
  const createAuthUseCase = new DbCreateAuthentication(usersRepository, hashComparer, encrypter, usersRepository)
  const createAuthentication = new CreateAuthenticationController(makeCreateAuthenticationValidation(), createAuthUseCase);
